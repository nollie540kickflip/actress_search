...
class IsarService {
  ...
  Future<List<Actress>> searchActresses({
    ...
    if (query.isNotEmpty) {
      final kataQuery = query.replaceAllMapped(RegExp(r'[ぁ-ゖ]'), (m) => String.fromCharCode(m.group(0)!.codeUnitAt(0) + 0x60));
      final hiraQuery = query.replaceAllMapped(RegExp(r'[ァ-ヶ]'), (m) => String.fromCharCode(m.group(0)!.codeUnitAt(0) - 0x60));
      
      filterQuery = filterQuery.and().group((q) => q
          .nameContains(query)
          .or()
          .rubyContains(kataQuery)
          .or()
          .rubyContains(hiraQuery)
      );
    }
